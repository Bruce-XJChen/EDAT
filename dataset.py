...
class ImageNet:
    ...
    def get_loader(batch_size, world_size, rank, root):
        ...
        #indices = np.random.choice(len(trainset_full), len(trainset_full) // 20, replace=False)
        ...
        #indices_test = np.random.choice(len(testset_full), len(testset_full) // 20, replace=False)
        #estset = Subset(testset_full, indices_test)
